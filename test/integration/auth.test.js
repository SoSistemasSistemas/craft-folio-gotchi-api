...
test.before(async () => {
  const response = await internalRequest.post(`${API_URL}`, testUser);
  testUser = Object.assign(testUser, response.body);
});

test('Login without email field', async (t) => {
  const { password } = testUser;
  const response = await internalRequest.post(`${API_URL}/login`, { password });
...
test('Login without password field', async (t) => {
  ...
  const response = await internalRequest.post(`${API_URL}/login`, { email });
...
test('Login credentials invalid', async (t) => {
  ...
  const response = await internalRequest.post(`${API_URL}/login`, { email, password });

  t.is(response.statusCode, BAD_REQUEST);
  t.is(response.body.error, 'Senha incorreta.');
});
test('Login successfully', async (t) => {
  const response = await internalRequest.post(`${API_URL}/login`, testUser);
...
test('Signup without email field', async (t) => {
  ...
  const response = await internalRequest.post(`${API_URL}`, { password });
...
test('Signup without password field', async (t) => {
  ...
  const response = await internalRequest.post(`${API_URL}`, { email });
...
test('Signup successfully', async (t) => {
  ...
  const response = await internalRequest.post(`${API_URL}`, { email, password });
  const { statusCode, body } = response;
